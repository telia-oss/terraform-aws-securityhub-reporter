...
os.environ["SECURITY_CONTROLS"] = "S3.2, S3.3, S3.8, EC2.1, EC2.18, EC2.9, EC2.3, EC2.7, RDS.1, RDS.2, RDS.4, RDS.3, DMS.1, IAM.1, IAM.8, Lambda.1"
...
class SecurityHubReporterTest(unittest.TestCase):
    ...
    def test_group_findings_by_control_id(self):
        ...
        by_ctrl_id = reporter.group_findings_by_control_id(findings)
        self.assertEqual(len(by_ctrl_id.keys()), len(findings), 'Incorrect number of keys')

    def test_build_metric_data(self):
        findings = create_one_unique_finding()
        by_ctrl_id = reporter.group_findings_by_control_id(findings)
        metric_data = reporter.build_metric_data(by_ctrl_id)['metric_data']
